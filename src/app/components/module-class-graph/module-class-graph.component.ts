...
export class ModuleClassGraphComponent implements OnInit, OnDestroy {
    ...
    private handleNodeClick(event: MouseEvent, node: RenderNode) {
        ...
        // Only DIRECTORY and FILE nodes should be expandable
        if (node.type === 'CLASS' || node.type === 'FUNCTION') return;

        const original = this.allNodesMap.get(node.id);
        if (!original || !original.children || original.children.length === 0) return;

        // FILTER: Only get DIRECTORY, FILE, and CLASS children (NO FUNCTION)
        const childrenToAdd = original.children
            .filter(c => c.type === 'DIRECTORY' || c.type === 'FILE' || c.type === 'CLASS')
            .map(c => this.createRenderNode(c, node.x, node.y));

        // Only expand if there are valid children to show
        if (childrenToAdd.length === 0) return;
